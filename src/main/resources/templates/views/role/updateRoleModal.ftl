...
<form id="addRoleFrom" class="layui-form layui-card-body">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="radio" disabled="disabled" name="roleLevel" value="0" title="系统" checked>
            <input type="radio" disabled="disabled" name="roleLevel" value="1" title="普通">
        </div>
    </div>
    <div class="layui-form-item">
    ...
    <div class="layui-form-item" id="resource_div">
        ...
        <div class="layui-input-block">
            <div id="update_role_resource" class="demo-tree-more"></div>
        </div>
    </div>
...
<script>
    layui.use(['tree', 'layer', 'form', 'util'], function () {
        ...
        layer.ready(function () {
            if (${role.roleLevel}==1){
                $("input[name=roleLevel][value=0]").attr("checked", false);
                $("input[name=roleLevel][value=1]").attr("checked", true);
            }
            ...
            if ($("#roleName").val != "" && $("#description").val() != "") {
                ...
                if (checkData.length == 0) { //判断复选框是否被选中
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/updateRole.json",
                        data: {id:id,roleName: roleName,description: description,roleLevel:roleLevel,roleIds: resourceList},
                        dataType: "json",
                        success: function (res) {
                            if (res.status == 0) {
